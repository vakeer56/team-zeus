...
export const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setAge(u.age !== null && u.age !== undefined ? u.age : '');
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-100 font-sans relative overflow-hidden bg-grid-pattern flex">
      ...
      {/* Sidebar - Consistent with Candidate Dashboard */}
      <aside className="w-64 border-r border-slate-900 bg-slate-950/60 backdrop-blur-md p-6 flex flex-col justify-between shrink-0 hidden md:flex z-20">
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[9px] text-slate-500 font-medium tracking-wide uppercase">AI-Powered Evaluations</span>
            </div>
          </div>
          ...
          <nav className="space-y-1">
            <button 
              ...
              className="w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold text-slate-400 hover:text-white rounded-xl hover:bg-slate-900 transition-all cursor-pointer text-left"
            >
            ...
            <button 
              ...
              className="w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold text-white bg-indigo-600/10 border border-indigo-500/20 rounded-xl cursor-pointer text-left"
            >
        ...
        <div className="space-y-4">
          <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-2.5 bg-gradient-to-r from-purple-950/10 to-indigo-950/10">
            ...
            <p className="text-[10px] text-slate-500 leading-relaxed">Your integrity. Our priority.</p>
          </div>
          
          <button 
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold text-slate-500 hover:text-rose-400 rounded-xl hover:bg-rose-500/5 transition-all cursor-pointer"
          >
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        ...
        <main className="max-w-3xl w-full mx-auto px-8 py-10 relative z-10 flex-grow">
          <div className="space-y-6">
            <div className="space-y-1">
              <h1 className="text-2xl font-extrabold font-['Outfit'] tracking-tight">Edit Profile Credentials</h1>
              <p className="text-slate-400 text-sm">Update your name, age, educational details, contact number, and credentials.</p>
            </div>

            <form onSubmit={handleUpdateProfile} className="space-y-6">
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-6 bg-slate-950/20">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider border-b border-slate-900 pb-2">Personal Information</h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-slate-900/50 border border-slate-850 rounded-xl text-xs text-slate-400 cursor-not-allowed opacity-70"
                      />
                    </div>
                  </div>
                  ...
                  <div className="sm:col-span-2 space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Educational Details</label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-500">
                      ...
                      <input 
                        ...
                        placeholder="B.S. in Computer Science, Stanford University (2025)"
                        className="w-full pl-10 pr-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-all"
                      />
                    </div>
                  </div>
              ...
              <div className="flex justify-end gap-4 pt-2">
                <button
                  type="button"
                  onClick={() => navigate('/candidate-dashboard')}
                  className="py-3 px-6 bg-slate-900 hover:bg-slate-850 text-slate-400 hover:text-white rounded-xl text-xs font-bold transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isSaving}
                  className="py-3 px-6 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg hover:shadow-indigo-600/20 cursor-pointer"
                >
